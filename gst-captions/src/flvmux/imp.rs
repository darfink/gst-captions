...
use crate::flvmux::amf::MessageName;
use crate::flvmux::caption::{CaptionError, CaptionTimeline, InputMode as CaptionInputMode};
use crate::flvmux::flv::MAXIMUM_TIMESTAMP_MS;
...
#[enum_type(name = "GstCaptionsFlvMuxInputMode")]
pub enum InputMode {
  #[default]
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq, glib::Enum)]
#[enum_type(name = "GstCaptionsFlvMuxMessageName")]
...
impl CaptionsFlvMux {
  ...
  fn load_text_interval(&self, state: &mut State) -> Result<Option<TextInterval>, gst::FlowError> {
    ...
    if gap {
      ...
      if crate::flvmux::caption::warn_if_amf_text_is_long(&text) {
        gst::warning!(
          CAT,
          imp = self,
  ...
  fn process_flv_buffer(&self, state: &mut State) -> Result<Vec<gst::Buffer>, gst::FlowError> {
    ...
    if settings.prime && !state.primed {
      let body = crate::flvmux::amf::script_data_body(settings.message_name.into(), "", None);
      output.push(gst::Buffer::from_mut_slice(
        crate::flvmux::flv::script_data_tag(
...
impl ObjectSubclass for CaptionsFlvMux {
  const NAME: &'static str = "GstCaptionsFlvMux";
  type Type = super::CaptionsFlvMux;
  ...
  fn with_class(class: &Self::Class) -> Self {
    let make_pad = |name: &str| {
      gst::Pad::builder_from_template(&class.pad_template(name).unwrap())
        ...
        .expect("captionsflvmux pad template must create AggregatorPad")
    };
    Self {
...
impl ObjectImpl for CaptionsFlvMux {
...
impl GstObjectImpl for CaptionsFlvMux {}

impl ElementImpl for CaptionsFlvMux {
...
impl AggregatorImpl for CaptionsFlvMux {
