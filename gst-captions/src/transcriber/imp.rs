...
/**
 * SECTION:element-captionstranscriber
 *
 ...
 *   captionstranscriber model-path=moonshine-tiny-streaming.gguf ! \
 *   fakesink dump=true
 * ```
 *
 ...
 * gst-launch-1.0 filesrc location=speech.wav ! wavparse ! audioconvert ! \
 *   audioresample ! \
 *   captionstranscriber model-path=whisper-large-v3.gguf mode=chunked \
...
pub struct CaptionsTranscriber {
  srcpad: gst::Pad,
  sinkpad: gst::Pad,
...
impl CaptionsTranscriber {
  ...
  fn sink_chain(
    ...
    let discont_threshold =
      gst::ClockTime::from_mseconds(self.settings.lock().unwrap().discont_threshold_ms as u64);
...
fn plan_gap_after_finalize(
  ...
  match duration {
    Some(duration) => {
      ...
      let gap = (end > uncovered_start).then(|| (uncovered_start, Some(end - uncovered_start)));
...
fn plan_timed_text(
  ...
  for (word, next_start) in words.into_iter().zip(next_starts) {
    ...
    let end = floor_end(
      ...
      next_start
        .map(|next_start| base_pts + gst::ClockTime::from_mseconds(next_start.max(0) as u64)),
    );
...
impl ObjectSubclass for CaptionsTranscriber {
  const NAME: &'static str = "GstCaptionsTranscriber";
  type Type = super::CaptionsTranscriber;
  ...
  fn with_class(klass: &Self::Class) -> Self {
    ...
    let sinkpad = gst::Pad::builder_from_template(&templ)
      .chain_function(|pad, parent, buffer| {
        CaptionsTranscriber::catch_panic_pad_function(
          parent,
...
impl ObjectImpl for CaptionsTranscriber {
  ...
  fn signals() -> &'static [glib::subclass::Signal] {
    static SIGNALS: LazyLock<Vec<glib::subclass::Signal>> = LazyLock::new(|| {
      vec![
          /**
           * GstCaptionsTranscriber::partial-transcript:
  ...
  fn set_property(&self, _id: usize, value: &glib::Value, pspec: &glib::ParamSpec) {
    ...
    match pspec.name() {
      ...
      "stable-prefix-agreement-n" => settings.stable_prefix_agreement_n = value.get().unwrap(),
...
impl GstObjectImpl for CaptionsTranscriber {}

impl ElementImpl for CaptionsTranscriber {
  fn metadata() -> Option<&'static gst::subclass::ElementMetadata> {
    static ELEMENT_METADATA: LazyLock<gst::subclass::ElementMetadata> = LazyLock::new(|| {
      gst::subclass::ElementMetadata::new(
        "Captions transcriber",
        "Audio/Text",
        "Speech-to-text filter backed by transcribe.cpp",
...
mod tests {
  ...
  fn source_pad_advertises_official_timed_text_caps() {
    gst::init().unwrap();
    let template = CaptionsTranscriber::pad_templates()
      .iter()
