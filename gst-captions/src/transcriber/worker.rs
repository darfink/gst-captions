    ...
    loop {
      ...
      match cmd {
        Cmd::Feed(pcm) => {
          ...
          let release_settled =
            !update.committed_changed && tracker.pending_is_settled(update.audio_committed_ms);
          ...
          if update.tentative_changed && ev_tx.send(Ev::Partial(stream.text().tentative)).is_err() {
            return Ok(());
          }
        }
        Cmd::Finalize => {
          match stream.finalize() {
            Ok(update) => {
              ...
              let drained_ms = update.audio_committed_ms.max(update.input_received_ms);
              let words =
                tracker.take_final(&text.committed, &commit::units(&snapshot), drained_ms);
