...
impl ObjectSubclass for CaptionsRollup {
  const NAME: &'static str = "GstCaptionsRollup";
  type Type = super::CaptionsRollup;
  type ParentType = gst::Element;

  fn with_class(klass: &Self::Class) -> Self {
    let sinkpad = gst::Pad::builder_from_template(&klass.pad_template("sink").unwrap())
      .chain_function(|pad, parent, buffer| {
        CaptionsRollup::catch_panic_pad_function(
          parent,
      ...
      .event_function(|pad, parent, event| {
        CaptionsRollup::catch_panic_pad_function(parent, || false, |imp| imp.sink_event(pad, event))
      })
    ...
    let srcpad = gst::Pad::builder_from_template(&klass.pad_template("src").unwrap())
      .query_function(|pad, parent, query| {
        CaptionsRollup::catch_panic_pad_function(parent, || false, |imp| imp.src_query(pad, query))
      })
      .flags(gst::PadFlags::PROXY_CAPS | gst::PadFlags::FIXED_CAPS)
      .build();
...
impl ObjectImpl for CaptionsRollup {
...
impl GstObjectImpl for CaptionsRollup {}

impl ElementImpl for CaptionsRollup {
